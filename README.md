# tf-atom-vpc-endpoint-service-aws

[![Terraform Format](https://img.shields.io/badge/terraform-fmt-blue?logo=terraform)](https://github.com/PlatformStackPulse/tf-atom-vpc-endpoint-service-aws/actions)
[![Terraform Validate](https://img.shields.io/badge/terraform-validate-blue?logo=terraform)](https://github.com/PlatformStackPulse/tf-atom-vpc-endpoint-service-aws/actions)
[![TFLint](https://img.shields.io/badge/tflint-passing-brightgreen?logo=terraform)](https://github.com/PlatformStackPulse/tf-atom-vpc-endpoint-service-aws/actions)
[![Terraform Test](https://img.shields.io/badge/tests-2%20passed-brightgreen?logo=terraform)](https://github.com/PlatformStackPulse/tf-atom-vpc-endpoint-service-aws/actions)
[![Security Scan](https://img.shields.io/badge/trivy-passing-brightgreen?logo=aqua)](https://github.com/PlatformStackPulse/tf-atom-vpc-endpoint-service-aws/actions)
...
[![Documentation](https://img.shields.io/badge/docs-terraform--docs-blue?logo=readthedocs)](https://github.com/PlatformStackPulse/tf-atom-vpc-endpoint-service-aws/actions)
[![License](https://img.shields.io/badge/license-MIT-blue?logo=opensourceinitiative)](LICENSE)

<!-- Badges -->

Terraform atom module that provisions an **AWS VPC Endpoint Service** (AWS PrivateLink service) fronted by Network or Gateway Load Balancers, with tf-label-driven naming and tagging.

## Features

- **VPC Endpoint Service (PrivateLink)** — Manages `aws_vpc_endpoint_service` so consumers can privately reach your service
- **NLB or GWLB backed** — Attach Network Load Balancer ARNs (`network_load_balancer_arns`) or Gateway Load Balancer ARNs (`gateway_load_balancer_arns`)
- **Acceptance control** — Toggle manual connection acceptance via `acceptance_required`
- **Allowed principals** — Whitelist consumer principal ARNs allowed to discover the service (`allowed_principals`)
- **Private DNS** — Optional `private_dns_name` for the endpoint service
- **IP address types** — Choose `ipv4`, `ipv6`, or both via `supported_ip_address_types`
- **tf-label naming & tags** — Consistent `namespace-environment-stage-name` IDs and standard tags via `module.this`
- **`enabled` toggle** — Set `enabled = false` to create no resources (context-chainable)
- **Native Terraform testing** — `terraform test` unit tests with a mock AWS provider (no AWS calls)
...
module "endpoint_service" {
  source = "git::https://github.com/PlatformStackPulse/tf-atom-vpc-endpoint-service-aws.git?ref=v1.0.0"
...
Consumers connect using the service name exposed via the `service_name` output. Use `gateway_load_balancer_arns` instead of `network_load_balancer_arns` when fronting the service with a Gateway Load Balancer.
...
|------|-------------|------|---------|:--------:|
| <a name="input_acceptance_required"></a> [acceptance\_required](#input\_acceptance\_required) | Whether VPC endpoint connection requests require acceptance. | `bool` | `true` | no |
| <a name="input_allowed_principals"></a> [allowed\_principals](#input\_allowed\_principals) | List of ARNs of principals allowed to discover the service. | `list(string)` | `[]` | no |
...
| <a name="input_gateway_load_balancer_arns"></a> [gateway\_load\_balancer\_arns](#input\_gateway\_load\_balancer\_arns) | ARNs of Gateway Load Balancers for the service. | `list(string)` | `[]` | no |
...
| <a name="input_network_load_balancer_arns"></a> [network\_load\_balancer\_arns](#input\_network\_load\_balancer\_arns) | ARNs of Network Load Balancers for the service. | `list(string)` | `[]` | no |
| <a name="input_private_dns_name"></a> [private\_dns\_name](#input\_private\_dns\_name) | Private DNS name for the service. | `string` | `null` | no |
| <a name="input_regex_replace_chars"></a> [regex\_replace\_chars](#input\_regex\_replace\_chars) | Terraform regular expression (regex) string.<br/>Characters matching the regex will be removed from the ID elements.<br/>If not set, `"/[^a-zA-Z0-9-]/"` is used to remove all characters other than hyphens, letters and digits. | `string` | `null` | no |
| <a name="input_stage"></a> [stage](#input\_stage) | ID element. Usually used to indicate role, e.g. 'prod', 'staging', 'source', 'build', 'test', 'deploy', 'release'. | `string` | `null` | no |
| <a name="input_supported_ip_address_types"></a> [supported\_ip\_address\_types](#input\_supported\_ip\_address\_types) | Supported IP address types. Valid values: ipv4, ipv6. | `list(string)` | <pre>[<br/>  "ipv4"<br/>]</pre> | no |
...
| Name | Description |
|------|-------------|
| <a name="output_arn"></a> [arn](#output\_arn) | The ARN of the VPC endpoint service. |
| <a name="output_enabled"></a> [enabled](#output\_enabled) | Whether the module is enabled. |
| <a name="output_id"></a> [id](#output\_id) | The ID of the VPC endpoint service. |
| <a name="output_service_name"></a> [service\_name](#output\_service\_name) | The service name for consumers. |
| <a name="output_state"></a> [state](#output\_state) | The state of the VPC endpoint service. |
<!-- END_TF_DOCS -->

## Tests

Unit tests use the native `terraform test` framework with a **mock AWS provider**, so no
real AWS credentials or resources are required. They assert on plan-known values only
(the tf-label `id`, planned resource count, and input pass-throughs) — computed
attributes such as the endpoint service `arn`/`id` are unknown under a mock provider.
...
|------|------------------|
| `creates_when_enabled` | One `aws_vpc_endpoint_service` is planned, inputs pass through, tf-label `id` is `eg-test-thing` |
| `disabled_creates_nothing` | `enabled = false` plans zero resources and `enabled` output is `false` |
